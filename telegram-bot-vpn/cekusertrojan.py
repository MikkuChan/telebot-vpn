...
# -*- coding: utf-8 -*-
"""
cekuser-trojan: Daftar user TROJAN (username, password/uuid, expired) + kirim ke Telegram
...
class TrojanUserChecker:
    ...
    def _extract_users(self, config_content):
        """Ekstrak daftar user dari konfigurasi Xray"""
        users = []
        lines = config_content.split('\n')
        
        for line in lines:
            # Cari baris yang dimulai dengan "#! " (komentar user)
    ...
    def _find_uuid_for_user(self, config_content, username):
        """Mencari UUID untuk user tertentu"""
        try:
            # Parse sebagai JSON untuk mendapatkan struktur yang tepat
            config_json = json.loads(config_content)
            
            # Cari dalam inbounds untuk konfigurasi trojan
            for inbound in config_json.get('inbounds', []):
                if inbound.get('protocol') == 'trojan':
                    ...
                    for client in clients:
                        # Biasanya ada email atau identifikasi lain
                        if username in str(client):
                            return client.get('password', 'N/A')
            ...
            for i, line in enumerate(lines):
                if re.search(pattern, line):
                    # Cari 2 baris ke depan untuk password
                    for j in range(i+1, min(i+3, len(lines))):
                        if '"password"' in lines[j]:
                            match = re.search(r'"password"\s*:\s*"([^"]+)"', lines[j])
                            if match:
    ...
    def _format_terminal_output(self, users_data):
        ...
        if not users_data:
            print("📋 Tidak ada user TROJAN yang ditemukan.")
            return
        
        # Header
        header = "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        title = "DAFTAR USER TROJAN"
        
        print(f"\n🔍 {title}")
        print(header)
        print(f"{'USERNAME':<20} {'PASSWORD/UUID':<36} {'EXPIRED':<12} {'STATUS':<10}")
        print("─" * 82)
    ...
    def _create_telegram_message(self, users_data):
        ...
        if not users_data:
            return "<b>📋 DAFTAR USER TROJAN</b>\n\n❌ Tidak ada user yang ditemukan."
        
        # Header pesan
        header = "<b>🔍 DAFTAR USER TROJAN</b>"
        separator = "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        
        # Header tabel
        table_header = f"<code>{'USERNAME':<16} {'PASSWORD/UUID':<36} {'EXPIRED':<10}</code>"
    ...
    def run(self):
        ...
        print("🚀 Memulai pengecekan user TROJAN...")
        ...
        # Ekstrak daftar user
        print("🔍 Mengekstrak daftar user...")
        users = self._extract_users(config_content)
        
        if not users:
            print("❌ Tidak ada user TROJAN yang ditemukan dalam konfigurasi.")
            return
